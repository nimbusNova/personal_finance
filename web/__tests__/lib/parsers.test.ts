...
describe('RobinhoodParser.parse', () => {
  ...
  it('parses account holder', () => {
    const r = parser.parse(syntheticText);
    expect(r.data?.account_holder).toBe('Michael Wu');
  });

  it('parses account number', () => {
    const r = parser.parse(syntheticText);
    expect(r.data?.account_number).toBe('562113779');
  });
...
describe('ChaseParser.parse', () => {
  ...
  it('parses account holder', () => {
    expect(parser.parse(syntheticText).data?.account_holder).toBe('MICHAEL WU');
  });

  it('parses account number', () => {
    expect(parser.parse(syntheticText).data?.account_number).toBe('000000106569236');
  });
...
describe('SchwabParser.parse', () => {
  ...
  it('parses account number', () => {
    expect(parser.parse(syntheticText).data?.account_number).toBe('7406-8532');
  });
